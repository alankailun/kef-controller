...
class EventTestPanel(QWidget):
    ...
    def _start_test(
        ...
        *,
        ...
        def worker() -> str:
            runner()
            return "The test event was sent."

        def failed(exc: Exception) -> None:
            detail = str(exc)
            self._log.info(f"TEST_EVENT_FAILED | label={label} | error={detail}")
            self._test_finished.emit(label, False, detail)

        start_background_task(
            f"UITest-{label.replace(' ', '')}",
            worker,
            on_success=lambda detail: self._test_finished.emit(label, True, detail),
            on_error=failed,
            log=self._log,
        )
