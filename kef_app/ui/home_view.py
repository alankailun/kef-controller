...
class HomeInterface(QWidget):
    ...
    def __init__(
        ...
        self._controller_bridge.speaker_state_changed.connect(self._on_external_state_fetched)
    ...
    def request_state_refresh(self) -> None:
        ...
        if self.isVisible() and self._event_poll_enabled():
            self._controller.start_speaker_event_monitor("ui_home_refresh")
        self.refresh()
        self._poll_external_state(force=self.isVisible())

    def showEvent(self, event: QShowEvent) -> None:
        super().showEvent(event)
        self._controller.start_speaker_event_monitor("ui_home_show")
    ...
    def _maybe_fetch_volume(self) -> None:
        ...
        start_background_task(
            "FetchVolume",
            self._controller.get_volume,
            on_success=self._vol_fetched.emit,
            on_error=lambda _exc: self._vol_fetched.emit(None),
    ...
    def _poll_external_state(self, force: bool = False) -> None:
        ...
        def run() -> None:
            return self._controller.poll_external_ui_state("ui_home_poll", "ui_home_poll")

        start_background_task(
            "PollExternalState",
            run,
            on_success=lambda result: self._external_state_fetched.emit(*result),
            lock=self._external_poll_lock,
            log=self._controller.log,
        )
    ...
    def _on_input_selected(self, idx: int) -> None:
        ...
        start_background_task(
            "UIChangeInput",
            run,
            on_success=lambda result: self._apply_done.emit(*result),
            on_error=lambda _exc: self._apply_done.emit(new_input, False),
            log=self._controller.log,
        )
    ...
    def _on_vol_value_changed(self, val: int) -> None:
        ...
        if self._vol_dragging:
            ...
            self._vol_debounce.start(max(50, int(self._config.home_volume_debounce_ms)))
    ...
    def _commit_volume(self) -> None:
        ...
        def run():
            return self._controller.set_volume(val)

        start_background_task(
            "UISetVolume",
            run,
            on_success=self._vol_set_done.emit,
            on_error=lambda _exc: self._vol_set_done.emit(False),
            log=self._controller.log,
        )
    ...
    def _run_power_action(
        ...
        start_background_task(thread_name, run, log=self._controller.log)
