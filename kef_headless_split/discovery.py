...
from .models import SpeakerIdentity, normalize_mac, normalize_model_label, normalize_name
...
def identify_kef_device(ip: str, config: AppConfig, log) -> Optional[SpeakerIdentity]:
...
def discover_kef_device_blind(known_mac: str, seed_ip: Optional[str], config: AppConfig, log) -> Optional[SpeakerIdentity]:
    ...
    for network in networks:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for identity in executor.map(lambda host_ip: identify_kef_device(host_ip, config, log), reachable_hosts):
                if not identity:
                    continue
